...
def myinput_num():
    global sInput
    print("请输入一个任意搜索条件(点用带括号的数字表示,'~'表示非 '&'表示合取 '|'表示析取 '>'表示蕴含 ':'表示等价 '@'表示异或,可用括号'()'):")
    sInput = input()
...
def getVariable_num():
    global sInput, variable
    origin_variable = re.split('[~&|()>:@]', sInput)
    # print(origin_variable)
    for c in origin_variable:
        if c not in variable and c != '':
            variable.append(c)
    variable = sorted(variable)
